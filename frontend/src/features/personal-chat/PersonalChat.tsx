...
export default function PersonalChat() {
  ...
  const MODE_PROMPTS: Record<string, string> = {
    widget: "\n\n[INSTRUCTION: Generate an interactive HTML/JS widget to visualize this concept. Enclose strictly in a ```jules-artifact``` block with an <artifact-title> at the top. CRITICAL RULES: 1) NEVER LEAVE VISUALS BLANK. Physics/Math: use <canvas> + Vanilla JS. Biology/Chem: use detailed inline <svg>. 2) You MUST import and use Alpine.js (<script defer src=\"https://cdn.jsdelivr.net/npm/alpinejs@3.x.x/dist/cdn.min.js\"></script>) for ALL UI state, sliders, and data binding (x-data, x-model, x-text) to ensure bug-free interactivity. 3) INTERACTIVE HOTSPOTS: SVG paths/visuals MUST have CSS hover effects and be clickable (e.g. @click). Clicking a part should update the side info panel dynamically via Alpine. 4) Use Tailwind CSS (dark mode, Bg: #08090c, violet accents). 5) Split layout: Visuals on one side, Controls/Info on the other. 6) NO 'math_inline' markdown in HTML, use standard unicode.]",
